...
const Board = () => {
    ...
    const { data:userMail, refetch } = useQuery({
        queryKey: ["userMail"],
        queryFn: async () => {
      ...
      const filteredUserMail = userMail?.filter((userData: { email: string }) => userData.email !== loggedInUser);
             ...
             <form onSubmit={(e) => handleSubmit(e)} className="p-4 md:p-5">
                <div className="grid gap-4 mb-4 grid-cols-2">
                    ...
                    <div className="col-span-2 sm:col-span-1">
                        <label htmlFor="member" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Member</label>
                        <select
  id="member"
  name="member"
...
>
  ...
  {filteredUserMail && filteredUserMail.map((userData: { email: string }, index: number) => (
