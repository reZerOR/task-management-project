...
const Board = ({ id }: BoardProps) => {
  ...
  const { data: isAdmin } = useQuery({
    queryKey: ["isAdmin"],
    queryFn: async () => {
      const res = await axiosPrivate.get(`/currentUserMail/${user?.email}`);
      // console.log("API response:", res.data); // Log the response
      // setUserEmail(res.data); // Assuming res.data is an array of User objects
  ...
  return (
    <div>
      {
        isAdmin && <div className="w-72 mx-auto flex items-center justify-center  col-span-3 md:col-span-12">
          <button
